  ...
  return (
    <section className={`py-12 sm:py-16 lg:py-24 relative overflow-hidden ${darkMode
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className={`absolute top-0 right-0 w-[400px] sm:w-[600px] lg:w-[800px] h-[400px] sm:h-[600px] lg:h-[800px] rounded-full mix-blend-screen filter blur-[60px] sm:blur-[100px] opacity-10 ${darkMode ? 'bg-amber-500' : 'bg-orange-300'
          }`} />
        <div className={`absolute bottom-0 left-0 w-[300px] sm:w-[600px] h-[300px] sm:h-[600px] rounded-full mix-blend-screen filter blur-[60px] sm:blur-[100px] opacity-10 ${darkMode ? 'bg-yellow-600' : 'bg-amber-200'
          }`} />
      ...
      <Container>
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-20 items-center relative z-10">
          ...
          >
            <div className="relative group scale-75 sm:scale-100 transition-transform duration-300">
              {/* Glowing Background */}
              ...
              <div className="relative w-64 h-64 sm:w-80 sm:h-80">
                ...
                <div className={`absolute inset-4 rounded-full flex items-center justify-center border-4 shadow-2xl ${darkMode
                  ...
                  <div className={`text-center ${darkMode ? 'text-amber-400' : 'text-orange-600'}`}>
                    <Award size={50} className="mx-auto mb-2 drop-shadow-lg sm:w-16 sm:h-16" />
                    <div className="text-2xl sm:text-3xl font-black tracking-tighter">ELITE</div>
                    <div className="text-xs sm:text-sm font-bold opacity-80 uppercase tracking-widest">Author</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className={`space-y-6 sm:space-y-8 transition-all duration-1000 delay-300 text-center lg:text-left ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'
            }`}>
            <div>
              ...
              <h2 className={`text-3xl sm:text-4xl lg:text-5xl font-black leading-tight mb-4 sm:mb-6 ${darkMode ? 'text-white' : 'text-slate-900'
                }`}>
                Proven Quality, <br />
              ...
              <p className={`text-sm sm:text-base lg:text-lg leading-relaxed max-w-2xl mx-auto lg:mx-0 ${darkMode ? 'text-slate-400' : 'text-slate-600'
                }`}>
            ...
            <div className="grid grid-cols-3 gap-2 sm:gap-4 border-y py-6 sm:py-8 border-slate-200 dark:border-slate-800">
              {stats.map((stat, index) => (
                <div key={index} className="text-center px-1">
                  <div className={`inline-flex p-2 sm:p-3 rounded-2xl mb-2 sm:mb-3 ${darkMode ? 'bg-slate-800 text-amber-400' : 'bg-orange-50 text-orange-600'
                    }`}>
                    <stat.icon size={16} className="sm:w-5 sm:h-5" />
                  </div>
                  <div className={`text-lg sm:text-2xl font-bold mb-0.5 sm:mb-1 ${darkMode ? 'text-white' : 'text-slate-900'
                    }`}>
                    {stat.value}
                  </div>
                  <div className={`text-[10px] sm:text-xs font-medium uppercase tracking-wider truncate ${darkMode ? 'text-slate-500' : 'text-slate-500'
                    }`}>
                    {stat.label}
                  </div>
                </div>
            ...
            <Button
              variant="primary"
              size="lg"
              className={`group w-full sm:w-auto px-8 h-12 sm:h-14 text-sm sm:text-base mx-auto lg:mx-0 ${darkMode
